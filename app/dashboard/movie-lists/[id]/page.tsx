...
export default function MovieListDetailPage({
  ...
  const [movieToDelete, setMovieToDelete] = useState<MovieListItem | null>(
    null
  );
  ...
  // Cargar lista de favoritos al inicio
  useEffect(() => {
    async function fetchFavorites() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Crear un Set con los IDs de las películas favoritas para búsqueda rápida
          const favIds = new Set(data.favorites.map((fav: any) => fav.id));
  ...
  async function removeFromList(movieId: string) {
    try {
      ...
      const response = await fetch(
        `/api/movie-lists/${unwrappedParams.id}/items/${movieId}`,
        {
          method: 'DELETE',
          headers: {
  ...
  async function toggleFavorite(movieId: string) {
    if (status !== 'authenticated') {
      toast({
        ...
        description:
          'Necesitas iniciar sesión para añadir películas a favoritos',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      if (favoriteMovieIds.has(movieId)) {
        ...
        if (response.ok) {
          ...
          const newFavorites = new Set(favoriteMovieIds);
          newFavorites.delete(movieId);
          setFavoriteMovieIds(newFavorites);

          toast({
            title: 'Película eliminada de favoritos',
            description: 'La película se ha quitado de tu lista de favoritos',
          });
        } else {
        ...
        if (response.ok) {
          ...
          toast({
            ...
            description: 'La película se ha añadido a tu lista de favoritos',
          });
        } else {
      ...
      toast({
        title: 'Error',
        description:
          'No se pudo actualizar favoritos. Inténtalo de nuevo más tarde.',
        variant: 'destructive',
      });
    } finally {
        ...
        <div className="container px-4 py-6">
          ...
          <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
            <div>
              ...
              <p className="mt-1 text-zinc-400">
                {movieCount} {movieCount === 1 ? 'film' : 'films'} • Created{' '}
                {movieList?.createdAt
          ...
          <p className="mt-4 max-w-3xl text-zinc-300">
            {movieList?.description ||
              'No description available for this list.'}
          </p>
        </div>
      </section>
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4 opacity-0 transition-opacity group-hover:opacity-100">
                    <div className="flex justify-between">
                      ...
                      <Button
                        ...
                        disabled={favoriteLoading === item.movieId}
                      >
                        {favoriteLoading === item.movieId ? (
        ...
        <div className="container px-4">
          ...
          <div className="mt-8 text-center">
            <p className="text-sm text-zinc-500">
              © 2025 Cult & Underground Movies. All rights reserved.
            </p>
          </div>
        </div>
      </footer>
        ...
        <DialogContent className="border-zinc-800 bg-zinc-900 text-white sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription className="text-zinc-400">
              Are you sure you want to remove "{movieToDelete?.movie.title}"
              from "{listTitle}"?
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <Button
              variant="destructive"
              onClick={() =>
                movieToDelete && removeFromList(movieToDelete.movieId)
              }
